...
export function evaluateTernaryExpression({node, ...rest}: EvaluatorOption<TernaryExpressionContext>) {
	const conditionalValue = evaluateNode({node: node.singleExpression(0), ...rest});
